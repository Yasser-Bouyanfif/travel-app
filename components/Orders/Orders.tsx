                ...
                {orders.map((order, index: number) => {
                    ...
                    return (
                        <div
                            ...
                            <button
                                onClick={new Date(item.startDate) > new Date() ? () => handleCancel(order._id) : undefined}
                                ...
                                className={`absolute top-4 right-4 text-white text-sm font-medium p-3 rounded-md z-10 cursor-pointer ${
                                    new Date(item.startDate) > new Date() ? 'bg-rose-600 hover:bg-rose-700' : 'bg-gray-400 cursor-not-allowed'
                                }`}
                            >
                                Cancel Booking
                            </button>
                            ...
                            <div className="p-6 md:w-1/2 flex flex-col justify-between">
                                <div>
                                    ...
                                    <div
                                        className="inline-block bg-indigo-50 text-indigo-600 px-3 py-1 rounded-md font-semibold text-lg mb-4">
                                        Reference: {item.ref}
                                    </div>
                                    ...
                                    <p className="text-gray-600">
                                        <span className="font-semibold">City: </span>{hotel.location}
                                    </p>
                                    <p className="text-gray-600">
                                ...
                                <div className="mt-4">
                                    <p className="text-gray-600">
                                        <span className="font-semibold">From: </span>{item.startDate}
                                    </p>
                                    <p className="text-gray-600">
                                        <span className="font-semibold">To: </span>{item.endDate}
                                    </p>
