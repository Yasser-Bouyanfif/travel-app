...
const SearchPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const data = res.filter(
          (stay: { hotel: Hotel }) => stay.hotel.location === location,
        );
        setHotels(data);
        setLoading(false);
      } catch (error) {
    ...
    fetchData();
  }, [startDate, endDate, location, router]);
  ...
  const toggleFavorite = (itemId: number) => {
    ...
    if (isAlreadyFavorite) {
      ...
      if (!startDate || !endDate) {
        toast.error("Start and end dates must be defined before favoriting.");
        return;
      }

      setFavorites((prev) => [
        ...prev,
        { id: itemId, startDate: startDate, endDate: endDate },
      ]);
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex space-x-2 items-center">
                      ...
                      >
                        <Heart
                          fill={
                            favorites.some((fav) => fav.id === obj.hotel.id)
                              ? "red"
                              : "none"
                          }
                          color="red"
